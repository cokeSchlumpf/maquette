...
public final class ProjectsRegistry extends EventSourcedBehavior<ProjectsMessage, ProjectsEvent, ProjectsRegistry.State> {
    ...
    private State onCreatedProject(State state, CreatedProject created) {
        state.getProjects().add(created.getProject());
        return state;
    }
    ...
    private State onDeletedProject(State state, DeletedProject deletedProject) {
        state.getProjects().remove(deletedProject.getProject());
        return state;
    }

    private Effect<ProjectsEvent, State> onListProjects(State state, ListProjects list) {
        list.getReplyTo().tell(ListProjectsResult.apply(state.getProjects()));
        return Effect().none();
    }
